...
public class EmpresaControllerTest extends EmpresaApplicationTests {
	...
	public void giveEmpresa_whenGetEmpresa_thenReturnList() throws Exception {
		mockMvc.perform(get("/empresas").contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
				.header("Authorization", getToken("ROLE_ADMIN")).content(getObjectMapper().writeValueAsString(empresa)))
	...
	public void giveEmpresa_whenGetEmpresaById_thenReturnEmpresa() throws Exception {
		mockMvc.perform(get("/empresas/" + empresa2.getId()).contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
	...
	public void giveEmpresa_whenPostEmprea_thenReturnStatusCreate() throws Exception {
		mockMvc.perform(post("/empresas/save").contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
				.header("Authorization", getToken("ROLE_ADMIN")).content(getObjectMapper().writeValueAsString(empresa)))
	...
	public void giveStore_whenPutEmpresa_thenReturnStatusOk() throws Exception {
		...
		mockMvc.perform(put("/empresas/update/" + id).contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
				.header("Authorization", getToken("ROLE_ADMIN"))
				.content(getObjectMapper().writeValueAsString(empresa3))).andExpect(status().isOk())
				.andExpect(jsonPath("$.nome").value(empresa3.getNome()))
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE));
	}

	@Test
	public void giveStore_whenDeleteEmpresa_thenReturnStatusOk() throws Exception {
		mockMvc.perform(delete("/empresas/" + empresa3.getId()).contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
				.header("Authorization", getToken("ROLE_ADMIN"))).andExpect(status().isOk());
	}
